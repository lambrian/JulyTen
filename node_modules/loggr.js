...
Loggr.prototype.getTotalInRange = function(query, lowerBound, upperBound) {
    ...
    return entriesList.toArray().reduce(function(p, c) {
        ...
        return (entry.get("date") >= lowerBound && entry.get("date") < upperBound) ? entry.get("value") + p : 0;
    }, 0);
}

Loggr.prototype.insertEntry = function(category, date, value) {
  this.entries.insert({
    category: category,
    date:     date,
    value:    value});
...
var initDatastore = function(error, datastore) {
    console.log("initting datastore");
    this.ds = datastore;
...
Loggr.prototype.startEngines = function() {
  Goodreads.start(this);
}
